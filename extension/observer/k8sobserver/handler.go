...
func (h *handler) OnAdd(objectInterface any, _ bool) {
	...
	case *v1.Pod:
		endpoints = convertPodToEndpoints(h.idNamespace, object, h.config.ListOnlyRunningPods)
	case *v1.Service:
...
func (h *handler) OnUpdate(oldObjectInterface, newObjectInterface any) {
	...
	case *v1.Pod:
		...
		for _, e := range convertPodToEndpoints(h.idNamespace, oldObject, h.config.ListOnlyRunningPods) {
			oldEndpoints[e.ID] = e
		}
		for _, e := range convertPodToEndpoints(h.idNamespace, newPod, h.config.ListOnlyRunningPods) {
			newEndpoints[e.ID] = e
		}
