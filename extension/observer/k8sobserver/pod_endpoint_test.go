...
func TestPodObjectToPortEndpoint(t *testing.T) {
	...
	endpoints := convertPodToEndpoints("namespace", podWithNamedPorts, true, TargetForPodIP)
	require.Equal(t, expectedEndpoints, endpoints)
}

func TestPodObjectToPortEndpointWithPodName(t *testing.T) {
	expectedEndpoints := []observer.Endpoint{
		{
			ID:     "namespace/pod-2-UID",
			Target: "default-pod-2",
			Details: &observer.Pod{
				Name:      "pod-2",
				Namespace: "default",
				UID:       "pod-2-UID",
	...
	endpoints := convertPodToEndpoints("namespace", podWithNamedPorts, true, TargetForPodName)
	require.Equal(t, expectedEndpoints, endpoints)
}
